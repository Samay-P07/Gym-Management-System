...
def index(request):
    if request.method == "POST":
        if 'uname' in request.POST:
            u = request.POST['uname'].strip()
            p = request.POST['pwd'].strip()
...
def registration(request):
    if request.method == "POST":
        ...
        if User.objects.filter(username=email).exists():
            messages.success(request, "This Email Address is already registered. Please try logging in.")
...
def view_messages(request):
    ...
    return render(request, "admin/view_messages.html", locals())